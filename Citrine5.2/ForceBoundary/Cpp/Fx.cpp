...
VectorXd Fx::fx() {
    ...
    // 定义一个计算 BCtemp 的 lambda 函数
        auto calculate_BCtemp0 = [](const VectorXd& Ynew, double Vx, double Vy, double Vz) {
        VectorXd BCtemp(5);
    ...
    // 定义一个计算 BCtemp 的 lambda 函数
        auto calculate_BCtemp1 = [](const VectorXd& Ynew, double Vx, double Vy, double Vz) {
        VectorXd BCtemp(5);
        double cos_theta = cos(Ynew(6));
        double sin_theta = sin(Ynew(6));
        double cos_phi = cos(Ynew(7));
        ...
        BCtemp(0) = Ynew(3) - (Vx * cos_phi * cos_theta + Vy * cos_theta * sin_phi - Vz * sin_theta);
        BCtemp(1) = Ynew(4) - (Vy * cos_phi - Vx * sin_phi);
        BCtemp(2) = Ynew(5) - (Vx * cos_phi * sin_theta + Vy * sin_theta * sin_phi + Vz * cos_theta);
        BCtemp(3) = Ynew(8);
        BCtemp(4) = Ynew(9);
        return BCtemp;
    };
    ...
    VectorXd BCtemp0 = calculate_BCtemp0(Ynew_segments[0], Vtx, Vty, Vtz);
    VectorXd BCtemp1 = calculate_BCtemp1(Ynew_segments[numSegments-1], Gbx, Gby, Gbz);
